[_tb_system_call storage=system/_1dayNight.ks]

*boutou

[playbgm  volume="100"  time="1000"  loop="true"  storage="insomnia.mp3"  ]
[bg  time="1000"  method="crossfade"  storage="living_room.PNG"  ]
[tb_show_message_window  ]
[popopo  type="sine"  volume="50"  time="20"  octave="0"  frequency="A"  chara="default"  ]
[tb_start_text mode=1 ]
#
リビングに戻りしばらくすると、やがて他のメンバーも戻ってきた。[p]
[_tb_end_text]

[bg  time="1000"  method="crossfade"  storage="調理場.PNG"  ]
[tb_start_text mode=1 ]
日も落ち始めた頃なので、すぐに夕飯の準備をしようという話になり、【料理人】を中心に、みんなで協力してご馳走を作った。[p]
みんなであれこれ言いながら料理を作っていると、時間が経つのはあっという間だった。[p]
気付けば、時刻はもう19時。[p]
料理の匂いをずっと嗅いでいたせいもあってか、全員すっかりお腹を空かせていた。[p]
[_tb_end_text]

[bg  time="1000"  method="crossfade"  storage="living_room.PNG"  ]
[tb_start_text mode=1 ]
完成した料理をテーブルに並べ、みんなで席につく。[p]
最初こそ「美味しい」「これ誰が作ったの？」なんて声も飛び交っていたが、それも束の間。[p]
いつの間にか会話は途切れ、みんな黙々と箸を進めていた。[p]
どうやら今は、恋愛よりも目の前のご飯らしい。[p]
そんなみんなの姿を見ていると何か安心感を覚えた。[p]
ここへ来ることに怯えていた自分が馬鹿馬鹿しく思えてくるようだった。[p]
みんな同じ人間、今考えれば当たり前のことだ。[p]
[_tb_end_text]

[chara_show  name="ダンサー"  time="300"  wait="true"  storage="chara/5/dancer1.PNG"  width="744"  height="1118"  left="243"  top="4"  reflect="false"  ]
[tb_start_text mode=1 ]
#ダンサー
うまかったー！[p]
久々にこんなうまいもん食ったわ！[p]
[_tb_end_text]

[chara_hide  name="ダンサー"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="音楽家"  time="300"  wait="true"  storage="chara/8/音楽家１.PNG"  width="732"  height="1099"  left="251"  top="36"  reflect="false"  ]
[tb_start_text mode=1 ]
#音楽家
その道のプロがいたからね。美味しくて当然だよ。[p]
[_tb_end_text]

[chara_hide  name="音楽家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="料理人"  time="300"  wait="true"  storage="chara/6/料理人１.PNG"  width="653"  height="984"  left="284"  top="49"  reflect="false"  ]
[tb_start_text mode=1 ]
#料理人
いいや、ベタな話かと思われるかもしれないが、みんなで作ったからさ。[p]
もちろんみんなで作ったって、料理そのものの味が変わるわけじゃないよ？[p]

[_tb_end_text]

[chara_mod  name="料理人"  time="600"  cross="true"  storage="chara/6/料理人２.PNG"  ]
[tb_start_text mode=1 ]
切り方が雑だったり、味付けで揉めたり、誰かが余計なことをして手間が増えたりもするし。[p]
それでも、出来上がったものを囲んで食べていると、不思議と美味しく感じる。[p]
誰が何を切ったとか、途中で何を失敗したとか。[p]

[_tb_end_text]

[chara_mod  name="料理人"  time="600"  cross="true"  storage="chara/6/料理人１.PNG"  ]
[tb_start_text mode=1 ]
そんな出来上がるまでの時間まで一緒に食べているから、美味しいんじゃないかな。[p]
[_tb_end_text]

[chara_hide  name="料理人"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="アイドル"  time="300"  wait="true"  storage="chara/7/0F411DC0-D4D8-434A-A669-8BFB49FF35D6.PNG"  width="701"  height="1051"  left="256"  top="-38"  reflect="false"  ]
[tb_start_text mode=1 ]
#アイドル
うわー！素敵です、その考え！[p]
どおりで美味しかったんだ！[p]
[_tb_end_text]

[chara_hide  name="アイドル"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="漫画家"  time="300"  wait="true"  storage="chara/4/漫画家４.PNG"  width="643"  height="966"  left="299"  top="27"  reflect="false"  ]
[tb_start_text mode=1 ]
#漫画家
一番掻き回してたんあんたやろ！[p]
[_tb_end_text]

[chara_hide  name="漫画家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="アイドル"  time="300"  wait="true"  storage="chara/7/idol４.PNG"  width="823"  height="1236"  left="196"  top="-65"  reflect="false"  ]
[tb_start_text mode=1 ]
#アイドル
えー！酷いです！あんなに頑張ったのに！[p]
[_tb_end_text]

[chara_hide  name="アイドル"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="医師"  time="300"  wait="true"  storage="chara/2/医師１.PNG"  width="743"  height="1112"  left="249"  top="2"  reflect="false"  ]
[tb_start_text mode=1 ]
#医師
まあ、やる気さえあれば何とかなる……と言えないのが料理の難しいところだね。[p]
【アイドル】さんのやる気は十分伝わってきてたよ。[p]
[_tb_end_text]

[chara_hide  name="医師"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="アイドル"  time="300"  wait="true"  storage="chara/7/0F411DC0-D4D8-434A-A669-8BFB49FF35D6.PNG"  width="701"  height="1051"  left="256"  top="-38"  reflect="false"  ]
[tb_start_text mode=1 ]
#アイドル
ありがとうございます！[p]
[_tb_end_text]

[chara_hide  name="アイドル"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="漫画家"  time="300"  wait="true"  storage="chara/4/漫画家２.PNG"  width="678"  height="1019"  left="274"  top="31"  reflect="false"  ]
[tb_start_text mode=1 ]
#漫画家
ポジティブ過ぎるやろ……[p]
[_tb_end_text]

[chara_hide  name="漫画家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="女優"  time="300"  wait="true"  storage="chara/1/女優v２.PNG"  width="824"  height="1239"  left="179"  top="-71"  reflect="false"  ]
[tb_start_text mode=1 ]
#女優
それにしてもいまだに不思議な感覚。[p]
【アイドル】さんや【漫画家】さんみたいな有名人とご飯食べてるなんて。[p]
[_tb_end_text]

[chara_hide  name="女優"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="医師"  time="300"  wait="true"  storage="chara/2/医師１.PNG"  width="743"  height="1112"  left="249"  top="2"  reflect="false"  ]
[tb_start_text mode=1 ]
#医師
いやいや、それはちょっと卑下が過ぎるんじゃない？[p]
君だってテレビでよく見かけるよ。[p]
[_tb_end_text]

[chara_hide  name="医師"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="ダンサー"  time="300"  wait="true"  storage="chara/5/dancer1.PNG"  width="744"  height="1118"  left="243"  top="4"  reflect="false"  ]
[tb_start_text mode=1 ]
#ダンサー
【女優】！俺もお前テレビで見たことあるぞ！[p]
何で見たか忘れたけど。だから心配すんな！[p]
[_tb_end_text]

[chara_hide  name="ダンサー"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="女優"  time="300"  wait="true"  storage="chara/1/女優１.PNG"  width="750"  height="1123"  left="213"  top="-38"  reflect="false"  ]
[tb_start_text mode=1 ]
#女優
言わせたみたいでごめんね。[p]
そんなつもりじゃなかったんだ。（笑）[p]
[_tb_end_text]

[chara_hide  name="女優"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="モデル"  time="300"  wait="true"  storage="chara/3/model１.PNG"  width="707"  height="1063"  left="243"  top="12"  reflect="false"  ]
[tb_start_text mode=1 ]
#モデル
まあでも実際この中で飛び抜けて有名って言ったら、【アイドル】さん、【漫画家】さん、【女優】さんってとこだと思うっす。[p]
俺なんかはいろんな雑誌ハシゴするのに精一杯って感じだし。[p]
[_tb_end_text]

[chara_hide  name="モデル"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="漫画家"  time="300"  wait="true"  storage="chara/4/漫画家２.PNG"  width="678"  height="1019"  left="274"  top="31"  reflect="false"  ]
[tb_start_text mode=1 ]
#漫画家
そんな二人と並べられても敵わんな〜。[p]
俺はたまたま一発当たっただけや。知名度がちゃうで。[p]
[_tb_end_text]

[chara_hide  name="漫画家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="小説家"  time="300"  wait="true"  storage="chara/9/小説家１.PNG"  width="705"  height="1059"  left="242"  top="38"  reflect="false"  ]
[tb_start_text mode=1 ]
#小説家
……そんなことないです。[p]
あの漫画は素晴らしかった。誇りに思ったほうがいいです……[p]
だから早く私のために絵を描いてください。[p]
[_tb_end_text]

[chara_hide  name="小説家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="漫画家"  time="300"  wait="true"  storage="chara/4/漫画家２.PNG"  width="678"  height="1019"  left="274"  top="31"  reflect="false"  ]
[tb_start_text mode=1 ]
#漫画家
また言うとるでこの子は……[p]

[_tb_end_text]

[chara_hide  name="漫画家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="音楽家"  time="300"  wait="true"  storage="chara/8/音楽家１.PNG"  width="732"  height="1099"  left="251"  top="36"  reflect="false"  ]
[tb_start_text mode=1 ]
#音楽家
またってどういうこと？[p]
[_tb_end_text]

[chara_hide  name="音楽家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="小説家"  time="300"  wait="true"  storage="chara/9/小説家１.PNG"  width="705"  height="1059"  left="242"  top="38"  reflect="false"  ]
[tb_start_text mode=1 ]
#小説家
私はあの漫画のファンです……[p]
絵を描いてくれたら宝物にする。朝からお願いしてるのに……[p]

[_tb_end_text]

[chara_hide  name="小説家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="医師"  time="300"  wait="true"  storage="chara/2/医師２.PNG"  width="724"  height="1080"  left="271"  top="6"  reflect="false"  ]
[tb_start_text mode=1 ]
#医師
【小説家】さん、気持ちは分かるんだけどプロ相手にそのお願いは失礼にあたるんじゃないかな……[p]

[_tb_end_text]

[chara_hide  name="医師"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="漫画家"  time="300"  wait="true"  storage="chara/4/漫画家２.PNG"  width="678"  height="1019"  left="274"  top="31"  reflect="false"  ]
[tb_start_text mode=1 ]
#漫画家
いやいや！そんなお高く止まってるつもりはないで！[p]

[_tb_end_text]

[chara_mod  name="漫画家"  time="600"  cross="true"  storage="chara/4/漫画家３.PNG"  ]
[tb_start_text mode=1 ]
ただ、画材もなんも持ってきてへんし思てな。お粗末なもん渡すわけにいかんし。[p]
[_tb_end_text]

[chara_hide  name="漫画家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="小説家"  time="300"  wait="true"  storage="chara/9/小説家１.PNG"  width="705"  height="1059"  left="242"  top="38"  reflect="false"  ]
[tb_start_text mode=1 ]
#小説家
そんなのなんでもいいです……！[p]
ボールペンと厚紙渡すんで描いて……！あと右下に私の名前を添えて。[p]
[_tb_end_text]

[chara_hide  name="小説家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="音楽家"  time="300"  wait="true"  storage="chara/8/音楽家１.PNG"  width="732"  height="1099"  left="251"  top="36"  reflect="false"  ]
[tb_start_text mode=1 ]
#音楽家
頼み方はアレだけど、描いてあげたら？[p]
彼女があんな興奮すること滅多にないよ。今日一日しか知らないけど。[p]
[_tb_end_text]

[chara_hide  name="音楽家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="漫画家"  time="300"  wait="true"  storage="chara/4/漫画家３.PNG"  width="733"  height="1101"  left="237"  top="26"  reflect="false"  ]
[tb_start_text mode=1 ]
#漫画家
まあええけど、そんな大したもんやないで……[p]
[_tb_end_text]

[chara_hide  name="漫画家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="小説家"  time="300"  wait="true"  storage="chara/9/小説家１.PNG"  width="735"  height="1107"  left="228"  top="58"  reflect="false"  ]
[tb_start_text mode=1 ]
#小説家
……[p]

[_tb_end_text]

[chara_mod  name="小説家"  time="600"  cross="true"  storage="chara/9/小説家５.PNG"  ]
[tb_start_text mode=1 ]
……[p]

[_tb_end_text]

[chara_hide  name="小説家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="女優"  time="300"  wait="true"  storage="chara/1/女優１.PNG"  width="750"  height="1123"  left="213"  top="-38"  reflect="false"  ]
[tb_start_text mode=1 ]
#女優
顔に出るタイプだ。[p]
[_tb_end_text]

[chara_hide  name="女優"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="音楽家"  time="300"  wait="true"  storage="chara/8/音楽家１.PNG"  width="732"  height="1099"  left="251"  top="36"  reflect="false"  ]
[tb_start_text mode=1 ]
#音楽家
ほら、あんな嬉しそうな【小説家】ちゃん見たことないよ。知らないけど。[p]
[_tb_end_text]

[chara_hide  name="音楽家"  time="300"  wait="true"  pos_mode="true"  ]
[tb_start_text mode=1 ]
#
ピンポンパンポーン……[p]
不意に館内にアナウンスが流れた。[p]
みな静まり返る。[p]
#アナウンス
一日目の共同生活、お疲れさまでした。初めて出会った人たちと過ごした今日一日。[p]
短い時間の中でも「この人のことをもっと知ってみたい」と感じた相手がいたのではないでしょうか。[p]
そこで、ここから一つだけ特別なイベントを行います。[p]
『ONE MESSAGE』ーーあなたの一言から、恋が動き出す。ーー[p]
一日目を振り返り、あなたが「もっと知りたい」と思った人を一人だけ選んでください。[p]
そして番組から用意されたメッセージカードに名前を添え、一言書いてください。書き終えたカードは、相手のロッカーへ。[p]
今日、一番「もっと知りたい」と思ったのは誰ですか？[p]
それではみなさん、筆をおとりください。[p]
ピンポンパンポーン……[p]
……[p]
[_tb_end_text]

[chara_show  name="女優"  time="300"  wait="true"  storage="chara/1/女優１.PNG"  width="750"  height="1123"  left="213"  top="-38"  reflect="false"  ]
[tb_start_text mode=1 ]
#女優
え？何ですか？メッセージ？[p]
[_tb_end_text]

[chara_hide  name="女優"  time="300"  wait="true"  pos_mode="true"  ]
[tb_start_text mode=1 ]
#あなた
（確かに机には全員分のメッセージカードとペンがある……）[p]
[_tb_end_text]

[chara_show  name="料理人"  time="300"  wait="true"  storage="chara/6/料理人１.PNG"  width="653"  height="984"  left="284"  top="49"  reflect="false"  ]
[tb_start_text mode=1 ]
#料理人
みたいだね。さあ、カードとペンを配っていくよ。[p]
[_tb_end_text]

[chara_hide  name="料理人"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="漫画家"  time="300"  wait="true"  storage="chara/4/漫画家２.PNG"  width="678"  height="1019"  left="274"  top="31"  reflect="false"  ]
[tb_start_text mode=1 ]
#漫画家
ええ、受け入れるの早ない？[p]
[_tb_end_text]

[chara_hide  name="漫画家"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="料理人"  time="300"  wait="true"  storage="chara/6/料理人１.PNG"  width="653"  height="984"  left="284"  top="49"  reflect="false"  ]
[tb_start_text mode=1 ]
#料理人
何だ、照れてるのかい？[p]
[_tb_end_text]

[chara_hide  name="料理人"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="漫画家"  time="300"  wait="true"  storage="chara/4/漫画家２.PNG"  width="678"  height="1019"  left="274"  top="31"  reflect="false"  ]
[tb_start_text mode=1 ]
#漫画家
いやそうやないけど……[p]
[_tb_end_text]

[chara_hide  name="漫画家"  time="300"  wait="true"  pos_mode="true"  ]
[tb_start_text mode=1 ]
#あなた
（強制参加のこういう企画、確かに見たことあるな……）[p]
[_tb_end_text]

[chara_show  name="アイドル"  time="300"  wait="true"  storage="chara/7/0F411DC0-D4D8-434A-A669-8BFB49FF35D6.PNG"  width="701"  height="1051"  left="256"  top="-38"  reflect="false"  ]
[tb_start_text mode=1 ]
#アイドル
うわー！なんかドキドキしますね！[p]
[_tb_end_text]

[chara_hide  name="アイドル"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="モデル"  time="300"  wait="true"  storage="chara/3/model１.PNG"  width="707"  height="1063"  left="243"  top="12"  reflect="false"  ]
[tb_start_text mode=1 ]
#モデル
まあでも一日目だけじゃ何とも言えないっすね……[p]
[_tb_end_text]

[chara_hide  name="モデル"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="医師"  time="300"  wait="true"  storage="chara/2/医師１.PNG"  width="743"  height="1112"  left="249"  top="2"  reflect="false"  ]
[tb_start_text mode=1 ]
#医師
まあ『もっと知りたい人』だから、そんな気を張らずに書けばいいんじゃないかな。[p]

[_tb_end_text]

[chara_hide  name="医師"  time="300"  wait="true"  pos_mode="true"  ]
[chara_show  name="料理人"  time="300"  wait="true"  storage="chara/6/料理人１.PNG"  width="653"  height="984"  left="284"  top="49"  reflect="false"  ]
[tb_start_text mode=1 ]
#料理人
みんな、人のは見ちゃいけないよ。[p]
[_tb_end_text]

[chara_hide  name="料理人"  time="300"  wait="true"  pos_mode="true"  ]
[tb_start_text mode=1 ]
#
もっと知りたい人か……[p]
誰に送ろう？[p]
[_tb_end_text]

[glink  color="btn_21_blue"  storage="1dayNight.ks"  size="40"  text="モデル"  target="*model"  x="550"  y="126"  width="174"  height="100"  _clickable_img=""  ]
[glink  color="btn_21_blue"  storage="1dayNight.ks"  size="40"  text="医師"  x="302"  y="263"  width="174"  height="100"  _clickable_img=""  target="*doctor"  ]
[glink  color="btn_21_blue"  storage="1dayNight.ks"  size="40"  text="漫画家"  x="547"  y="426"  width="174"  height="100"  _clickable_img=""  target="*mangaka"  ]
[glink  color="btn_21_blue"  storage="1dayNight.ks"  size="40"  text="ダンサー"  x="768"  y="259"  width="216"  height="100"  _clickable_img=""  target="*dancer"  ]
[glink  color="btn_21_red"  storage="1dayNight.ks"  size="40"  text="料理人"  x="299"  y="126"  width="174"  height="100"  _clickable_img=""  target="*ryourinin"  ]
[glink  color="btn_21_red"  storage="1dayNight.ks"  size="40"  text="アイドル"  x="291"  y="427"  width="210"  height="100"  _clickable_img=""  target="*idol"  ]
[glink  color="btn_21_red"  storage="1dayNight.ks"  size="40"  text="音楽家"  x="543"  y="271"  width="174"  height="100"  _clickable_img=""  target="*ongakuka"  ]
[glink  color="btn_21_red"  storage="1dayNight.ks"  size="40"  text="小説家"  x="780"  y="423"  width="174"  height="100"  _clickable_img=""  target="*shousetsuka"  ]
[glink  color="btn_21_red"  storage="1dayNight.ks"  size="40"  text="女優"  x="783"  y="126"  width="174"  height="100"  _clickable_img=""  target="*joyuu"  ]
[s  ]
*model

[tb_start_text mode=1 ]
『【モデル】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【モデル】のロッカーにカードを入れた。[p]
【モデル】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.model_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
*doctor

[tb_start_text mode=1 ]
『【医師】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【医師】のロッカーにカードを入れた。[p]
【医師】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.doctor_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
*mangaka

[tb_start_text mode=1 ]
『【漫画家】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【漫画家】のロッカーにカードを入れた。[p]
【漫画家】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.mangaka_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
*dancer

[tb_start_text mode=1 ]
『【ダンサー】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【ダンサー】のロッカーにカードを入れた。[p]
【ダンサー】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.dancer_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
*ryourinin

[tb_start_text mode=1 ]
『【料理人】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【料理人】のロッカーにカードを入れた。[p]
【料理人】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.ryourinin_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
*idol

[tb_start_text mode=1 ]
『【アイドル】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【アイドル】のロッカーにカードを入れた。[p]
【アイドル】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.idol_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
*ongakuka

[tb_start_text mode=1 ]
『【音楽家】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【音楽家】のロッカーにカードを入れた。[p]
【音楽家】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.ongakuka_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
*shousetsuka

[tb_start_text mode=1 ]
『【小説家】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【小説家】のロッカーにカードを入れた。[p]
【小説家】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.shousetsuka_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
*joyuu

[tb_start_text mode=1 ]
『【女優】さん。機会があればまた話してみたいです。』[p]
一人ずつ部屋を出ていきロッカーにカードを入れていく。[p]
あなたは順番が来たので【女優】のロッカーにカードを入れた。[p]
【女優】からの好感度に変動がありました。[p]
[_tb_end_text]

[tb_start_tyrano_code]
[eval exp="f.joyuu_like += 3"]
[_tb_end_tyrano_code]

[jump  storage="event1.ks"  target="*jamp"  ]
